...
const { input, numberInput, renderSettings, run, selectConcept } = require('../src/index.js');
...
test('runs the URL-to-video flow and writes outputs and a summary', async () => {
  ...
  await writeFile(eventPath, JSON.stringify({ repository: { homepage: 'https://example.com' } }));
  const requests = [];
  const responses = [
